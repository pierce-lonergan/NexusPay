...
- ~~**B-002-activation | tx-manager + system routing + job routing + FORCE (C5-C7)**~~ | DONE 2026-06-13 (dormant, CI-green; commits 719ee55 + f06d483). The machinery is LANDED behind `rls.enforce=false`: single EMF over a `RoleRoutingDataSource` (app/system Hikari pools) + `RlsRoutingTransactionManager.doBegin` set_config GUC; `@SystemTransactional` (relocated to `io.nexuspay.common.rls`) + `SystemRoleAspect` thread-local role pin; the 15 genuinely cross-tenant @Scheduled jobs annotated SYSTEM; broken `TenantAwareDataSourceConfig` deleted; `R__rls_force_owner.sql` repeatable FORCE (gated by `${rlsforce}`, default false). Proven by RlsDormancyIT (everything absent + no table forced at enforce=false) + RlsEnforceIT (boots on nexuspay_app under the `rls-enforce` profile: tenant isolation, fail-closed-unbound, @SystemTransactional bypass, every RLS table FORCE'd). See ADR-012, L-033/34/35.
- **B-002-activation-tenant | bind TenantContext on the 6 TENANT_SCOPED consumers + per-item billing writes** | T3 security | HIGH | MANDATORY PRE-FLIP. The C6 classification found 6 single-tenant Kafka consumers that NEVER set TenantContext, so under enforcement they run on nexuspay_app with no GUC → fail-closed (zero rows / WITH CHECK reject) and BREAK at the flip. Each must bind TenantContext from the event's tenant_id (try/finally) and stay on the APP role (NOT @SystemTransactional): (1) `ledger/.../PaymentEventConsumer.onPaymentEvent` (idempotency read + posting INSERT die → DLT); (2) `analytics/.../PaymentEventAnalyticsConsumer.consume`; (3) `analytics/.../RoutingEventAnalyticsConsumer.consume`; (4) `billing/.../BillingPaymentEventListener.onPaymentEvent` (invoices stop marking PAID, dunning stops); (5) `gateway-api/.../WebhookDeliveryService.onPaymentEvent` (ALSO needs @Transactional + a tenant-scoped finder instead of findAllByEnabledTrue); (6) analytics `FraudEventAnalyticsConsumer` is NO_DB today but pre-bind when it gains persistence. PLUS the billing batch sweeps (RenewalScheduler doProcessRenewals/processDunning, TrialManagementService.convertExpiredTrials) run the DISCOVERY scan as SYSTEM but must bind TenantContext per-subscription for the per-item WRITES so WITH CHECK still guards them (split the one cross-tenant tx into per-tenant txns) — TrialManagementService was reclassified SYSTEM→TENANT_SCOPED for exactly this. Also: DeadLetterReprocessor's async whenComplete saves run on the producer thread (off the SYSTEM pin) — harmless today (dead_letter_queue has no RLS) but refactor before adding RLS there. Each fix needs an IT under the rls-enforce profile proving the consumer still works. See L-035, C6 workflow result.
